...
**
** Summary: Main function of the HomeAutomation application. Contructs main window and shows it.
**
****************************************************************************/
...
/***
 ** Create main Application object and window
 ** Load main stylesheet for the application
 ** Additional stylesheets are loaded on a per widget basis
 ** Hide cursor - we are using a touchscreen
 **/

int transition_speed;
int sensitivity_temp;
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<argc; i++)
    {
        ...
        {
            if(currentName.contains(QString("transitions")))
            {
                transition_speed=currentVal.toInt();
            }else if(currentName.contains(QString("sensitivity"))){
                TemperatureControl::sensitivity = currentVal.toInt();
            }
