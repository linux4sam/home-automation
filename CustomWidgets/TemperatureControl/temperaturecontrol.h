...
public:
    explicit TemperatureControl(QWidget *parent = 0);
    ~TemperatureControl();
    void setMode(int mode);
    void setHeatingCooling(int deviceMode);
    void deviceOn(bool heat);
    void deviceOff(bool heat);
    QSize sizeHint() const;

    static int sensitivity;
    static float unitsSensivity[3];
...
private:
    void setupDial();
    QString getValue(int temp);
    void adjustingTemperature();
    ...
    qreal currAngle, prevAngle;
    int mImageWidth, mImageHeight, mRadius, mKnobCenterX, mKnobCenterY;

    int currentTemperature;
    int targetTemperature;
    int displayUnits; // 0 - Fahrenheit, 1 - Celsius, 2 - % -humidity
    bool adjustingNow;
    bool sameTemps;
    int devMode;
};
