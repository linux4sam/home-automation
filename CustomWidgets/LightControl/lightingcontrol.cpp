...
**
** Summary: Top class for the light control Widget
**
****************************************************************************/

#include "lightingcontrol.h"
#include "ui_lightingcontrol.h"
 ...
 **/
LightingControl::LightingControl(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LightingControl)
{
    ui->setupUi(this);

    setupDial();

    connect(ui->lightBrightenss, SIGNAL(dimLightSignal()), this, SLOT(dimLight()));
    connect(ui->lightBrightenss, SIGNAL(increaseBrightnessSignal()), this, SLOT(increaseBrightness()));

    QFile qss(":/resources/lighting-control-dial.qss");
...
/***
 ** Initial Dial setup
 ** One can change dial sensitivity - default = 10
 **/
void LightingControl::setupDial()
{
    //init everything;
    up = 0;
    down = 0;
...
/***
 ** Method that handles light dimming
 **/
void LightingControl::dimLight()
{
    down++;
    if (down >= sensitivity)
    {
...
/***
 ** Method that increases light brightness
 **/
void LightingControl::increaseBrightness()
{
    up++;
    if (up >= sensitivity)
    {
...
/***
 ** Method that handles ON/OFF button
 ** placed in the center of a knob
 **/
void LightingControl::on_onOffButton_clicked()
{
    if (ui->lightBrightenss->onOff) {
        ui->lightBrightenss->onOff = false;
        emit(switchOnOff(false));
    } else {
        ui->lightBrightenss->onOff = true;
        emit(switchOnOff(true));
    }
    ui->lightBrightenss->repaint();
}

/***
 ** Method that handles light brightness
 **/
void LightingControl::adjustingBrightness()
{
    /* Service Routine for the Brightness */
    //TODO: Add code to handle dimming of an external device e.g. GPIO connected
    //Brightness value is stored in the local "brightness" variable
