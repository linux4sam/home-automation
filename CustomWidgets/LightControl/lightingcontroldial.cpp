...
**
** Summary: Custom control widget for light. Widget includes a round dial that rotates to control
**          light brightess that is also indicated on the embedded rim. Center of the dial knob
**          is a button that controls ON/OFF state of a controled device
**
****************************************************************************/
 ...
 ** Default constructor
 **/
LightingControlDial::LightingControlDial(QWidget *parent) :
...
/***
 ** Initial Dial Construction... Dial images are in an SVG format hence
 ** using QScgRenderer
 **/
void LightingControlDial::setupDial()
{
    rotationRange = 360;
    setRange(0, 360);
    onOff = true;
    dialImage = QPixmap::fromImage(QImage(":/resources/lighting-dial-nocolor.png"));
    update();
}

/***
 ** Custom, overriden paintEvent that handles rotation and displaying of
 ** all SVG image layers
 **/
void LightingControlDial::paintEvent(QPaintEvent *)
{
    int val = value();
    ...
    QPen pen(QColor("#f59719"), 20, Qt::SolidLine, Qt::FlatCap);
    painter.setPen( pen );
    painter.translate(width() / 2, height() / 2);

    int smaller_length = qMin(width(), height());
    painter.scale(smaller_length / 240.0, smaller_length / 240.0);

    double paintAngle = 360.0 / rotationRange;


    painter.save();

    painter.rotate(val * paintAngle ) ;

    QRectF frame(QPointF(-70, -70), QPointF(70, 70));

    if (onOff)
        painter.drawArc(frame, 90*16, val*16);

    painter.drawPixmap(-dialImage.width() * .5, -dialImage.height() * .5, dialImage);
    painter.restore();
}
 ...
 **/
void LightingControlDial::mouseMoveEvent(QMouseEvent * event)
{
    if (event->buttons() & Qt::LeftButton)
    {
        QLineF zeroLine(width()/2, height()/2, width()/2, 0);
        QLineF currentLine(width()/2, height()/2, event->x(), event->y());
        newAngle = currentLine.angleTo(zeroLine);

        int sliderNewPosition = (newAngle * rotationRange) / 360.0;

        if (sliderNewPosition)
        {
            if (newAngle > prevAngle)
               emit increaseBrightnessSignal();

            if (newAngle < prevAngle)
                emit dimLightSignal();
        }
        setValue(sliderNewPosition);
        prevAngle = newAngle;
    }
}
