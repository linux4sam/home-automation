...
**
** Summary: Single day weather detail
**
****************************************************************************/
 ...
 ** Default constructor
 **/
DayWeatherDetail::DayWeatherDetail(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::DayWeatherDetail)
{
    ui->setupUi(this);
...
/***
 ** Store weather handler pointer in a local variable
 **/
void DayWeatherDetail::setHandler(WeatherXmlContentHandler* xmlHandler)
{
    handler = xmlHandler;
}

/***
 ** Set weather values based on the data from the weather handler
 **/
void DayWeatherDetail::setWeatherDetailValues()
{
    ui->date_label->setText(QString("Today is %1").arg(QDate::currentDate().toString()));
    ui->humidity_label->setText(QString("Humidity: %1%").arg(handler->humidity));
    ui->precip_label->setText(QString("Precipitation: %1%").arg(handler->precipitation));
    ui->temp_label->setText(QString("%1F/%2F").arg(handler->maximumTemps.value(0)).arg(handler->minimumTemps.value(0)));
    QIcon icon(handler->weatherIconLocations.value(0));
    ui->weather_icon_label->setPixmap( icon.pixmap( DayWeather::sLargeIconSize));
    ui->weather_description_label->setText(handler->weatherForecastDescription);
    ui->dewpoint_label->setText(QString("Dew Point: %1F").arg(handler->dewPoint));
    ui->wind_label->setText(QString("Wind: %1 Knots").arg(handler->sustainedWind));
}
