...
QSize DayWeather::sNormalIconSize(70,70);
QSize DayWeather::sLargeIconSize(158,158);

/***
 ** Default constructor
 ** Loads custom stylesheet
 **/
DayWeather::DayWeather(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::DayWeather)
{
    ui->setupUi(this);
...
/***
 ** When left button pressed on widget switch to 5 day forcast
 **/
void DayWeather::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        emit(show5DayForecast());
    }
}

/***
 ** Set the weather data based on the weather handler
 **/
void DayWeather::setData(WeatherXmlContentHandler* handler)
{
    QIcon icon(handler->weatherIconLocations.value(0));
    ui->day->setText( (getDayString(QDate::currentDate().dayOfWeek() - 1)).toUpper() );
    ui->temperature->setText(QString("%2F / %1F").arg(handler->minimumTemps.value(0)).arg(handler->maximumTemps.value(0)));
    ui->weatherIcon->setPixmap( icon.pixmap( sNormalIconSize));
}

/***
 ** Retun name of the week
 **/
QString DayWeather::getDayString(int day)
{
    return WEEKDAY_NAMES[day % 7];
}

/***
 ** Set text on the day label for the widget
 **/
void DayWeather::setDay(QString day)
{
    ui->day->setText(day);
}

/***
 ** Set temperature on the label for widget
 **/
void DayWeather::setTemperature(QString temp)
{
    ui->temperature->setText(temp);
}

void DayWeather::setCompactView( bool compact)
{
    if (compact) {
        ui->verticalLayout->setContentsMargins(0,0,0,0);
        ui->verticalLayout->setSpacing(1);
        //ui->verticalLayout->removeWidget( ui->day);
        //ui->day->hide();
        ui->verticalLayout->removeWidget( ui->temperature);
        ui->temperature->hide();
    } else {
        ui->verticalLayout->setContentsMargins(0,13,0,0);
        ui->verticalLayout->setSpacing(3);
        //ui->verticalLayout->addWidget( ui->day);
        //ui->day->show();
        ui->verticalLayout->addWidget( ui->temperature);
        ui->temperature->show();
    }
}
