...
**
** Summary: Weather Widget class

 * @todo integrate zip code from HA settings into weather widget
 * @todo integrate radar into weather widget
 * @todo integrate f/c detection
 *
 * @todo move all #includes to header files
 * @todo add doxygen comments after integration
 * @todo possible add loading text while waiting for network connection
 *
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Creates an instance of a weather XML donwloader/handler
    ...
    ui(new Ui::WeatherWidget)
{
    ...
    if (isSmallResolution) {
        ...
        stylePath = ":/resources/weather/resources/weather-stylesheet-small.qss";
    } else {
        stylePath = ":/resources/weather/resources/weather-stylesheet.qss";
    }
...
/***
 ** Update weather info based on the ZIP code
 **/
void WeatherWidget::updateWeather(int zip)
{
    zipCode = zip;
...
/***
 ** Download weather details using the handler
 **/
void WeatherWidget::getWeatherInformation()
{
    connect(networkAccessManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(zipCodeFileIsReady(QNetworkReply*)) );
    networkAccessManager->get(QNetworkRequest(QUrl(QString("http://maps.googleapis.com/maps/api/geocode/xml?components=postal_code:%1&sensor=false").arg(zipCode))));
}

/***
 ** Data for the given ZIP code is available so parse the file
 **/
void WeatherWidget::zipCodeFileIsReady( QNetworkReply * reply )
{
    bool ok = parseFile(getTempFileName(reply));

    if (!ok)
    {
        std::cout << "Parsing failed; reading local XML feed at " << LOCAL_XML_FEED.toStdString() << " instead." << std::endl;
        fillWeather(LOCAL_XML_FEED);
    }
    else
    {
        std::cout << "Parsing succeeded. Got zip code coords from Google: " << handler->latitudes.value(0) << ", " << handler->longitudes.value(0) << ". Put these into weather.gov." << std::endl;
        disconnect(networkAccessManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(zipCodeFileIsReady(QNetworkReply*)));
        connect(networkAccessManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(weatherFileIsReady(QNetworkReply*)) );

        networkAccessManager->get(QNetworkRequest(QUrl(QString("http://forecast.weather.gov/MapClick.php?lat=%1&lon=%2&unit=0&lg=english&FcstType=dwml").arg(handler->latitudes.value(0)).arg(handler->longitudes.value(0)) )));
    }
}

/***
 ** Local temp file for storing weather data
 **/
QString WeatherWidget::getTempFileName( QNetworkReply * reply )
{
    xml_file = new QTemporaryFile();
    ...
    QString fileName = xml_file->fileName();

    xml_file->write(reply->readAll());
    xml_file->setAutoRemove(false);
    xml_file->close();
...
/***
 ** Populate widget with data when its ready
 **/
void WeatherWidget::weatherFileIsReady( QNetworkReply * reply )
{
    fillWeather(getTempFileName(reply));
}

/***
 ** Actual function that handles weather info in the widget
 **/
void WeatherWidget::fillWeather( const QString & fileName )
{
    bool ok = parseFile(fileName);

    if (!ok)
    {
        std::cout << "Parsing failed." << std::endl;
        if (!fileName.compare(LOCAL_XML_FEED))
        {
            std::cout << "Reading local XML feed at " << LOCAL_XML_FEED.toStdString() << "failed. File not compatible or file missing/corrupted" << std::endl;
            return;
        }
        else
        {
            std::cout << "Downloaded XML file not compatible. Using local XML feed." <<std::endl;
            fillWeather(LOCAL_XML_FEED);
        }
    }
    else
    ...
    setSingleWeatherData(ui->dayOneForecast, 0);
    setSingleWeatherData(ui->dayTwoForecast, 1);
    setSingleWeatherData(ui->dayThreeForecast, 2);
    setSingleWeatherData(ui->dayFourForecast, 3);
    setSingleWeatherData(ui->dayFiveForecast, 4);

    if (!fileName.compare(LOCAL_XML_FEED))
        handler->stateShortName = "local";

//    std::cout << handler->stateShortName.toStdString() << std::endl;
    disconnect(networkAccessManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(weatherFileIsReady(QNetworkReply*)) );

    emit weatherXmlHandlerIsReady();
}

/***
 ** Set data for a single weather widget
 **/
void WeatherWidget::setSingleWeatherData(DayWeather* dayItem, int dayNum)
{
    ImageWidget* dayLabel;
    int day = QDate::currentDate().dayOfWeek() - 1;
    dayItem->setDay(getDayString(day+dayNum).toUpper());
    dayLabel = dayItem->findChild<ImageWidget *>("weatherIcon");
    QIcon icon(handler->weatherIconLocations.value(dayNum));
    dayLabel->setPixmap( icon.pixmap( dayNum == 0 ? DayWeather::sLargeIconSize :
                                                    DayWeather::sNormalIconSize));
    dayItem->setTemperature( QString("%1F / %2F").arg(handler->maximumTemps.value(dayNum)).arg(handler->minimumTemps.value(dayNum)) );
}

/***
 ** Weather data file parser
 **/
bool WeatherWidget::parseFile( const QString & fileName )
{
    ...
    if (!xmlFile->remove())
        std::cout << "Temp file not removed successfully." << std::endl;

    return ret;
}

/***
 ** Return string for an int "day" in a week
 **/
QString WeatherWidget::getDayString(int day)
{
    return WEEKDAY_NAMES[day % 7];
}
