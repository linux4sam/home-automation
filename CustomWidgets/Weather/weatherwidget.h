...
const QString WEEKDAY_NAMES [] = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
const QString LOCAL_XML_FEED = ":/resources/weather/weatherfeed.xml";
...
public:
    ...
    ~WeatherWidget();
    WeatherXmlContentHandler* getHandler() { return handler; }
    void getWeatherInformation();

signals:
    void weatherXmlHandlerIsReady();

public slots:
    void updateWeather(int zip);

private slots:
    void weatherFileIsReady( QNetworkReply * reply );
    void zipCodeFileIsReady( QNetworkReply * reply );
private:
    QString getTempFileName( QNetworkReply * reply );
    void fillWeather( const QString & fileName );
    bool parseFile( const QString & fileName );
    void setSingleWeatherData(DayWeather* dayItem, int day);

    enum daysofweek_t {MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY};
    DayWeatherDetail *myDayWeatherDetail;
    Ui::WeatherWidget *ui;
    QTemporaryFile *xml_file;
    WeatherXmlContentHandler* handler;
    QString getDayString(int day);
    int zipCode;
