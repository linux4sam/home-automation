...
/***
 ** Default constructor
 ** Set a placeholder image when no camera capture
 **/
    ...
    QWidget(parent)
{
    ...
    if (isSmallResolution) {
        image = QImage(":/smallresources/window-security.png");
    } else {
        image = QImage(":/resources/window-security.png");
    }
...
/***
 ** Set a picture (camera captured) under pixmap variable
 **/
...
/***
 ** Device name to handle by current thread e.g. /dev/video0
 **/
 ...
 ** Get device name
 **/
QString videowidget::getDeviceName()
{
    return thread.getDeviceName();
}
