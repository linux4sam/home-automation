...
{
    ui->setupUi(this);
    connect(&ui->videoViewer->thread, SIGNAL(streamIsReady()), this, SLOT(startNewStream()));
}
...
void CameraCapture::changeEvent(QEvent *e)
{
...
/***
 ** start Video playback of the captured stream of pictures
 **/
void CameraCapture::onStartVideo(QString device)
{
    if(!ui->videoViewer->thread.isRunning())
    {
        ui->videoViewer->setDeviceName(device);
        ui->videoViewer->thread.startCapture();

    }
    else if (ui->videoViewer->thread.isRunning() &&  ui->videoViewer->getDeviceName().compare(device))
    {

        ui->videoViewer->setDeviceName("");
        ui->videoViewer->thread.stopCapture();
    }
}

/***
 ** Halt playback of the video feed
 ** Reset video playback picture
 **/
void CameraCapture::stopVideo()
{
    ui->videoViewer->setDeviceName("");
    ui->videoViewer->thread.stopCapture();
    QImage image(":/resources/window-security.png");
    ui->videoViewer->setPicture(image);

}

/***
 ** Start playback of the camera feed
 **/
void CameraCapture::startNewStream()
{
    ui->videoViewer->thread.startCapture();
}
