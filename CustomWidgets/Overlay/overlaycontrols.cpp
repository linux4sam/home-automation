...
**
** Summary: C++ class that handles QML code for overlay controls
**
****************************************************************************/

#include "overlaycontrols.h"
#include "ui_overlaycontrols.h"

/***
 ** Constructor
 ** Create a QDeclarativeView object and load the Overlay.qml
 ** Add the new widget to the UI layout (left hand side)
 **/
OverlayControls::OverlayControls(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OverlayControls)
{
    ui->setupUi(this);
    ...
    declView->setResizeMode(QDeclarativeView::SizeViewToRootObject);
    ...
    if (isSmallResolution) {
        ...
        setGeometry(0, 0, width() * wSFactor, height() * hSFactor);

        setMinimumSize(minimumWidth() * wSFactor, minimumHeight() * hSFactor);
        setMaximumSize(maximumWidth() * wSFactor, maximumHeight() * hSFactor);

        this->setStyleSheet("background-color: #19262E");

        declView->setSource(QUrl("qrc:/qml/smallOverlay/Overlay.qml"));
    } else {
        declView->setSource(QUrl("qrc:/qml/Overlay/Overlay.qml"));
        this->setStyleSheet("background-color: #19262E");
    }

    ui->verticalLayout->addWidget(declView);
}
...
/***
 ** Handling function for the Climate button
 ** Connects QML generated signals with the rest of the QT application
 ** C++ code
 **/
//void OverlayControls::onClimateButtonPressed ()
//{
//    emit(climateRequest());
//}

/***
 ** Handling function for the Media button
 ** Connects QML generated signals with the rest of the QT application
 ** C++ code
 **/
//void OverlayControls::onMediaButtonPressed ()
//{
//    emit(mediaRequest());
//}

/***
 ** Handling function for the Lighting button
 ** Connects QML generated signals with the rest of the QT application
 ** C++ code
 **/
//void OverlayControls::onLightingButtonPressed ()
//{
//    emit(lightingRequest());
//}

/***
 ** Handling function for the Security button
 ** Connects QML generated signals with the rest of the QT application
 ** C++ code
 **/
//void OverlayControls::onSecurityButtonPressed ()
//{
//    emit(securityRequest());
//}

/***
 ** After the Ovelay.qml object is loaded and ready, connect signals between
 ** C++ and QML code
 **/
void OverlayControls::onViewStatusChanged(QDeclarativeView::Status status)
{
    if (status == QDeclarativeView::Ready)
    {
        connect ((QObject *)declView->rootObject(), SIGNAL(onClimateButtonPressed()),
                 this, SIGNAL(climateRequest()));
        connect ((QObject *)declView->rootObject(), SIGNAL(onSecurityButtonPressed()),
                 this, SIGNAL(securityRequest()));
        connect ((QObject *)declView->rootObject(), SIGNAL(onLightingButtonPressed()),
                 this, SIGNAL(lightingRequest()));
        connect ((QObject *)declView->rootObject(), SIGNAL(onMediaButtonPressed()),
                 this, SIGNAL(mediaRequest()));

        if (isSmallResolution) {
            connect ((QObject *)declView->rootObject(), SIGNAL(onHomeButtonPressed()),
                     this, SIGNAL(homeRequest()));
        }
    }
}
