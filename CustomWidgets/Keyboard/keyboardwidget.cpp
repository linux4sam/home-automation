...
**
** Summary: On screen keyboard. Used to enter text in QEditLine widgets
** Acknowledgment: Keyboard layout is similar to the keyboard available in meego project
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Create a fixed size rectangle that will be populated with a keyboard
 ** Keybaord code is written in QML hence handling the code loading process here
 **/
KeyboardWidget::KeyboardWidget(QWidget */*parent*/) :
    QWidget(0,Qt::Window | Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint),
                                                                declView(0),
                                                                editField(0)
{
  setAttribute(Qt::WA_ShowWithoutActivating);

    keyboardLayout = new QHBoxLayout(this);
    ...
    declView = new QDeclarativeView;
    declView->setResizeMode(QDeclarativeView::SizeRootObjectToView);
    keyboardLayout->addWidget(declView);

    connect (declView,SIGNAL(statusChanged(QDeclarativeView::Status)),this,
             SLOT(onViewStatusChanged(QDeclarativeView::Status)));

    if (isSmallResolution) {
        declView->setSource(QUrl("qrc:/qml/smallkeyboard/keyboard.qml"));
    } else {
        declView->setSource(QUrl("qrc:/qml/keyboard/keyboard.qml"));
    }
}
...
/***
 ** When Enter key is pressed, paste entered string to teh active window field and destroy the keyboard object.
 **/
void KeyboardWidget::onEnterButtonPressed (const QString& string)
{
    QKeyEvent			*key_event_press;
    QKeyEvent			*key_event_release;
    QKeyEvent			*key_event_string_post;
    QKeyEvent			*key_event_clear;

        key_event_press = new QKeyEvent(QEvent::KeyPress, Qt::Key_Enter, Qt::NoModifier);
        key_event_release = new QKeyEvent(QEvent::KeyRelease, Qt::Key_Enter, Qt::NoModifier);
        key_event_string_post = new QKeyEvent(QEvent::KeyPress, 0, Qt::NoModifier, string);
        key_event_clear = new QKeyEvent(QEvent::KeyPress, 0, Qt::NoModifier, QString(""));


    if (editField)
    {
        QCoreApplication::postEvent(editField, key_event_clear);
        QCoreApplication::postEvent(editField, key_event_string_post);
        QCoreApplication::postEvent(editField, key_event_press);
        QCoreApplication::postEvent(editField, key_event_release);
        editField->parentWidget()->setFocus();
        editField->removeEventFilter(this);
    }

    setCurrentString(QString(" "));
//    this->deleteLater();
    emit(killKeyboard());
}

/***
 ** After the keyboard.qml code is loaded and ready, connect the Enter signal between C++ and QML code
 **/
void KeyboardWidget::onViewStatusChanged(QDeclarativeView::Status status)
{
    if (status == QDeclarativeView::Ready)
    {
        connect ((QObject *)declView->rootObject(), SIGNAL(sigEnterButtonPressed(const QString&)),
                 this, SLOT(onEnterButtonPressed(const QString&)));
    }
}

/***
 ** Set pointer to the receiving field for the keyboard
 **/
void KeyboardWidget::setReceivingField(QWidget *object)
{
    if (editField)
    {
        editField->removeEventFilter(this);
    ...
    editField = object;
    editField->installEventFilter(this);

}

/***
 ** Take the text from teh current field in C++ code e.g. QEditLine and pass it to the keyboard for editing
 **/
void KeyboardWidget::setCurrentString(const QString& string)
{
    QVariant retValue;
    QMetaObject::invokeMethod((QObject *)(declView->rootObject()), "presetText", Q_RETURN_ARG(QVariant, retValue), Q_ARG(QVariant, string));
}
