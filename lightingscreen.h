...
class LightingScreen : public QWidget
{
    Q_OBJECT
    
public:
    explicit LightingScreen(QWidget *parent = 0);
    ~LightingScreen();
    void switchNames(int index);
    
public slots:
    void setLighingRoomNames( QList<QStringList> &my2DArray);

protected:
    void resizeEvent(QResizeEvent*);

private slots:
    void switchLight1(bool status);
    void switchLight2(bool status);
    void switchLight3(bool status);
    void onZoneChanged(QString);

private:
    Ui::LightingScreen *ui;
    QList<QStringList> roomNames;
    QStringList mZoneList;
    ZoneSelector *mZoneSelector;
};

#endif // LIGHTINGSCREEN_H
