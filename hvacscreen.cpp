...
**
** Summary: Qt UI handling class for the HVAC set of views. Please consult hvacscreen.ui file for
**          object definitions. Multiple Screens for HVAC controls and Weather are embedded and
**          manage through this class
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Creates object for children Widgets that include:
 ** - Weather Widget screen
 ** - Day Weather Detail screen
 ** - Doppler Radar screen
 ** - XML handler that downloads weather info (when online)
 **
 ** Connects signals with handlers for defined objects
 **/
HVACScreen::HVACScreen(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::HVACScreen)
{
    ui->setupUi(this);

    ui->stackedHVACWidget->setSpeed(500);
    ui->stackedHVACWidget->setVerticalMode(false);
    ui->stackedHVACWidget->setCurrentIndex(0);       //set the initial page to HVAC page

    hvacControl = new HVACSet();
    ui->layoutHVACControl->addWidget(hvacControl);

    weatherForecastWidget = new WeatherForcast();
    ui->layoutWeather5Day->addWidget(weatherForecastWidget);
    WeatherWidget* weatherWidget = weatherForecastWidget->getWeatherWidget();

    WeatherDetail* weatherDetailWidget = new WeatherDetail();
    ui->layoutWeatherDetail->addWidget(weatherDetailWidget);
    ...
    connect(hvacControl, SIGNAL(requestWeatherForecast()), this, SLOT(switchToWeatherForcast()));
...
/***
 ** Switch the view in the HVAC Screen to 5 Day Weather Forcast
 **/
void HVACScreen::switchToWeatherForcast()
{
    ui->stackedHVACWidget->setCurrentIndex(1);
}

/***
 ** Switch the view in the HVAC Screen to HVAC Control screen
 **/
void HVACScreen::switchToHVAC()
{
    ui->stackedHVACWidget->setCurrentIndex(0);
    emit(requestLabelsforHVAC());
}

/***
 ** Switch the view in the HVAC Screen to Today's Weather Details
 **/
...
/***
 ** Sets names of HVAC zones based on information from the Settings screen
 **/
void HVACScreen::setHVACZonesInfo(QStringList& list)
{
    hvacControl->setZoneLabels(list);
}
