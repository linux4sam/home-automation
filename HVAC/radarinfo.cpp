...
**
** Summary: Qt UI handling class for the Radar info screen. Please consult radarinfo.ui file for
**          object definitions
**
****************************************************************************/

#include "radarinfo.h"
#include "ui_radarinfo.h"
...
/***
 ** Default contructor
 ** @brief RadarInfo::RadarInfo
 ** @param parent
 **
 ** @note big states do not display radar correctly because the name in the CNN url does not match the 2 letter state name abbrev.
 **/
RadarInfo::RadarInfo(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RadarInfo)
{
    ui->setupUi(this);
    ...
    ui->webView->setStyleSheet("background-color: #19262E");
}
...
/***
 ** Handler for the Left arrow button - switching to a different page
 **/
void RadarInfo::on_pushButtonLeft_clicked()
{
    emit(requestWeatherDetails());
}

/***
 ** Store weather handler in a local variable
 **/
void RadarInfo::setHandler(WeatherXmlContentHandler* newHandler)
{
    handler = newHandler;
}

/***
 ** Show the animated weather gif for the given ZIP code
 ** If no network connectivity available display a local file instead
 **/
void RadarInfo::setRadarGif()
{
    QString shortName = handler->stateShortName.toLower();
    //Temporary fix for CA and TX radar maps.
    QString suffixName = "_";
    if(!shortName.compare("ca"))
        suffixName = "c";
    else if(!shortName.compare("tx"))
        suffixName = "s";

    QUrl url(QString("http://i.cdn.turner.com/cnn/.element/img/3.0/weather/maps/nxrd_%1%2.gif").arg(shortName).arg(suffixName));

    if( shortName.compare(QString("local")) != 0 )
        ui->webView->load( url );
    else //Use local radar gif
    {
        QFile::copy(LOCAL_RADAR_GIF, "/tmp/local-radar.gif");
        ui->webView->load(QUrl::fromLocalFile("/tmp/local-radar.gif"));

        //Clean up temp file created above.
        QFile file("/tmp/local-radar.gif");
        file.remove();
    }
}
