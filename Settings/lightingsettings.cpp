...
**
** Summary: Qt UI handling class for the lighting settings screen. Please consult lightingsettings.ui file for
**          object definitions
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Preset the initial Zone/Room names
 **/
LightingSettings::LightingSettings(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LightingSettings)
{
    ui->setupUi(this);

    QStringList list1;
    list1 << QString("Main Floor");
    localSettings << list1;
    QStringList list2;
    list2 << QString("Second Floor");
    localSettings << list2;
    QStringList list3;
    list3 << QString("Basement");
    localSettings << list3;

    QStringList* defaultList = (QStringList*)(&(localSettings[0]));
    defaultList->append(QString("Living Room"));
    defaultList->append(QString("Dining Room"));
    defaultList->append(QString("Family Room"));

    ui->lineEdit_Room1->setText(defaultList->at(1));
    ui->lineEdit_Room2->setText(defaultList->at(2));
    ui->lineEdit_Room3->setText(defaultList->at(3));
    ...
    if (isSmallResolution) {
        ...
        f = ui->selectZone->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->selectZone->setFont(f);

        f = ui->lineEdit_Room1->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room1->setFont(f);

        f = ui->lineEdit_Room2->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room2->setFont(f);

        f = ui->lineEdit_Room3->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room3->setFont(f);

        f = ui->lineEdit_Room4->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room4->setFont(f);

        f = ui->lineEdit_Room5->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room5->setFont(f);

        f = ui->lineEdit_Room6->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_Room6->setFont(f);

        ui->horizontalLayout_2->setSpacing(ui->horizontalLayout_2->spacing() * hSFactor);
        ui->verticalLayout_3->setSpacing(6);
        ui->verticalLayout_4->setSpacing(6);
        ui->verticalLayout_2->setSpacing(6);

        ui->verticalLayout_2->setContentsMargins(ui->verticalLayout_2->contentsMargins().left() * wSFactor,
                                                 ui->verticalLayout_2->contentsMargins().right() * wSFactor,
                                                 ui->verticalLayout_2->contentsMargins().bottom() * hSFactor,
                                                 ui->verticalLayout_2->contentsMargins().top() * hSFactor);

        ui->pushButtonSave->setMaximumSize(ui->pushButtonSave->maximumWidth() * wSFactor,
                                           ui->pushButtonSave->maximumHeight() * hSFactor);

        ui->pushButtonSave->setIconSize(QSize(ui->pushButtonSave->iconSize().width() * wSFactor,
                                              ui->pushButtonSave->iconSize().height() * hSFactor));

        QIcon icon;
        icon.addFile(QString::fromUtf8(":/resources/other/smallbutton-save.png"), QSize(), QIcon::Normal, QIcon::Off);
        ui->pushButtonSave->setIcon(icon);

    } else {
        QIcon icon;
        icon.addFile(QString::fromUtf8(":/resources/other/button-save.png"), QSize(), QIcon::Normal, QIcon::Off);
        ui->pushButtonSave->setIcon(icon);
    }
}
...
/***
 ** Populate QEditLine fields with preset settings
 **/
void LightingSettings::setZones(QStringList &zonesList)
{
    ui->selectZone->clear();

    for(int i=0; i<zonesList.size(); i++)
    {
        QString zone = zonesList.at(i);
        if(!(zone.isEmpty())){
            ui->selectZone->addItem(zone);
            if(localSettings.size()>i)
                localSettings[i].replace(0, zone);
            else{
                QStringList list;
                list << zone;
                localSettings.append(list);
            }
        }
    }
}
...
/***
 ** Load differnt set of Room names when zone is changed
 **/
void LightingSettings::on_selectZone_currentIndexChanged(int index)
{
    if(!localSettings.isEmpty() && index >= 0)
    {
        QStringList* defaultList = (QStringList*)(&(localSettings[index]));
        if(defaultList->count() > 1)
            ui->lineEdit_Room1->setText(defaultList->at(1));
        else
            ui->lineEdit_Room1->setText(QString(""));

        if(defaultList->count() > 2)
            ui->lineEdit_Room2->setText(defaultList->at(2));
        else
            ui->lineEdit_Room2->setText(QString(""));

        if(defaultList->count() > 3)
            ui->lineEdit_Room3->setText(defaultList->at(3));
        else
            ui->lineEdit_Room3->setText(QString(""));
    }
 ...
 ** Update data
 **/
void LightingSettings::updateData()
{
    updateLightingRoomNames();
    emit(roomNamesChanged(localSettings));
}

/***
 ** Save changes to Room names in a local structure
 **/
void LightingSettings::updateLightingRoomNames()
{
    int index = ui->selectZone->currentIndex();

    while(localSettings[index].count()>1)
        localSettings[index].removeAt(1);

    localSettings[index].append(ui->lineEdit_Room1->text());
    localSettings[index].append(ui->lineEdit_Room2->text());
    localSettings[index].append(ui->lineEdit_Room3->text());
}

/***
 ** Handler for the save button
 **/
void LightingSettings::on_pushButtonSave_clicked()
{
    updateLightingRoomNames();
}
