...
**
** Summary: Qt UI handling class for the Climate settings. Please consult climatesettings.ui file for
**          object definitions
**
****************************************************************************/


#include "climatesettings.h"
#include "ui_climatesettings.h"

/***
 ** Default constructor
 **/
ClimateSettings::ClimateSettings(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ClimateSettings)
{
    ui->setupUi(this);
    ui->lineEdit_ClimateZone1->clearFocus();
    ...
    if (isSmallResolution) {
        ...
        f = ui->lineEdit_ClimateZone1->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone1->setFont(f);

        f = ui->lineEdit_ClimateZone2->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone2->setFont(f);

        f = ui->lineEdit_ClimateZone3->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone3->setFont(f);

        f = ui->lineEdit_ClimateZone4->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone4->setFont(f);

        f = ui->lineEdit_ClimateZone5->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone5->setFont(f);

        f = ui->lineEdit_ClimateZone6->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone6->setFont(f);

        f = ui->lineEdit_ClimateZone7->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone7->setFont(f);

        f = ui->lineEdit_ClimateZone8->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ClimateZone8->setFont(f);

        ui->verticalLayout->setContentsMargins(ui->verticalLayout->contentsMargins().left() * wSFactor,
                                               ui->verticalLayout->contentsMargins().right() * wSFactor,
                                               ui->verticalLayout->contentsMargins().bottom() * hSFactor,
                                               ui->verticalLayout->contentsMargins().top() * hSFactor);
        ...
        ui->pushSaveButton->setMaximumSize(ui->pushSaveButton->maximumWidth() * wSFactor,
                                           ui->pushSaveButton->maximumHeight() * hSFactor);

        ui->pushSaveButton->setIconSize(QSize(ui->pushSaveButton->iconSize().width() * wSFactor,
                                              ui->pushSaveButton->iconSize().height() * hSFactor));

        QIcon icon;
        icon.addFile(QString::fromUtf8(":/resources/other/smallbutton-save.png"), QSize(), QIcon::Normal, QIcon::Off);
        ui->pushSaveButton->setIcon(icon);
...
/***
 ** Store zone entries in a local structure, emit a signal when done
 **/
QStringList& ClimateSettings::getZones()
{
    while(!climateZonesList.isEmpty())
        climateZonesList.removeFirst();

        climateZonesList << (ui->lineEdit_ClimateZone1->text());
        climateZonesList << (ui->lineEdit_ClimateZone2->text());
        climateZonesList << (ui->lineEdit_ClimateZone3->text());
        climateZonesList << (ui->lineEdit_ClimateZone4->text());
        climateZonesList << (ui->lineEdit_ClimateZone5->text());
        climateZonesList << (ui->lineEdit_ClimateZone6->text());
        climateZonesList << (ui->lineEdit_ClimateZone7->text());
        climateZonesList << (ui->lineEdit_ClimateZone8->text());

        emit(sendingZonesInfo(climateZonesList));

        return climateZonesList;
}
 ...
 ** Store zone entries in a local structure, emit a signal when done
 **/
void ClimateSettings::on_pushSaveButton_clicked()
{
    while(!climateZonesList.isEmpty())
        climateZonesList.removeFirst();
