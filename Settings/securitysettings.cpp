...
**
** Summary: Qt UI handling class for the Security settings. Please consult securitysettings.ui file for
**          object definitions
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Set default values for QEditLine fields
 **/
SecuritySettings::SecuritySettings(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::SecuritySettings)
{
    ui->setupUi(this);

    securityZonesList << "Front Door";
    securityZonesList << "Back Door";
    securityZonesList << "Garage";

    ui->lineEdit_ZONE1->setText(securityZonesList.at(0));
    ui->lineEdit_ZONE2->setText(securityZonesList.at(1));
    ui->lineEdit_ZONE3->setText(securityZonesList.at(2));
    ...
    if (isSmallResolution) {
        ...
        f = ui->lineEdit_ZONE1->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ZONE1->setFont(f);

        f = ui->lineEdit_ZONE2->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ZONE2->setFont(f);

        f = ui->lineEdit_ZONE3->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ZONE3->setFont(f);

        f = ui->lineEdit_ZONE4->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ZONE4->setFont(f);

        f = ui->lineEdit_ZONE5->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->lineEdit_ZONE5->setFont(f);
        ...
        ui->verticalLayout_3->setSpacing(ui->verticalLayout_3->spacing() * hSFactor);
        ui->verticalLayout_4->setSpacing(ui->verticalLayout_4->spacing() * hSFactor);

        layout()->setSpacing(layout()->spacing() * hSFactor);
        layout()->setContentsMargins(layout()->contentsMargins().left() * wSFactor,
                                     layout()->contentsMargins().right() * wSFactor,
                                     layout()->contentsMargins().bottom() * hSFactor,
                                     layout()->contentsMargins().top() * hSFactor);
...
/***
 ** Save info on security Zones in a local structure
 **/
void SecuritySettings::updateSecurityZONES()
{
    while(securityZonesList.count()>1)
        securityZonesList.clear();

    securityZonesList.append(ui->lineEdit_ZONE1->text());
    securityZonesList.append(ui->lineEdit_ZONE2->text());
    securityZonesList.append(ui->lineEdit_ZONE3->text());
}

/***
 ** Handler for a Save button
 **/
void SecuritySettings::on_pushButtonSave_clicked()
{
    updateSecurityZONES();
    emit(securityZonesChanged(securityZonesList));
}
