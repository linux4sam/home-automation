...
**
** Summary: Qt UI handling class for the Preference settings. Please consult preferencesettings.ui file for
**          object definitions
**
****************************************************************************/


#include "prefssettings.h"
#include "ui_prefssettings.h"
#include <iostream>

/***
 ** Default constructor
 **/
int PrefsSettings::zipCode = DEFAULT_ZIPCODE;

PrefsSettings::PrefsSettings(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::PrefsSettings)
{
    ui->setupUi(this);
    ...
    if (isSmallResolution) {
        ...
        QFont f = ui->prefsLabel->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->prefsLabel->setFont(f);

        f = ui->timeZoneLabel->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->timeZoneLabel->setFont(f);

        f = ui->zipCodeLabel->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->zipCodeLabel->setFont(f);

        f = ui->temperatureLabel->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->temperatureLabel->setFont(f);
        ...
        f = ui->temperature->font();
        f.setPointSize(f.pointSize() * hSFactor);
        ui->temperature->setFont(f);
...
/***
 ** Public method to obtain a zip code
 **/
int PrefsSettings::getZipCode()
{
    return zipCode;
}

/***
 ** Handling function when zipcode get changed by a user
 **/
void PrefsSettings::on_zipCode_textChanged(const QString &arg1)
{
    bool ok;
    int tempZipCode = arg1.toInt(&ok);

    if(ok && arg1.size() == 5)
    {
        zipCode = tempZipCode;
        std::cout << "hi" << zipCode << std::endl;

        emit(zipCodeChanged(zipCode));
    }
}

/***
 ** Handling function when temperature is changed F -> C or oposite
 **/
void PrefsSettings::on_temperature_currentIndexChanged(int index)
{
    emit(temperatureChanged(index));
}

/***
 ** Handler for the save button
 **/
void PrefsSettings::on_pushButtonSave_clicked()
{
    emit(temperatureChanged(ui->temperature->currentIndex()));
    emit(zipCodeChanged(zipCode));
}
