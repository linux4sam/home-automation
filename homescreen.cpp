...
**
** Summary: Qt UI handling class for the Home screen. Please consult homescreen.ui file for
**          object definitions. This is the first screen user sees. All major features
**          can be accessed from this screen
**
****************************************************************************/
...
/***
 ** Default constructor
 ** Connects signals for Help and Settings to MainControls object
 ** @parent - pointer to MainWindow object
 **
 **/
    ...
    ui(new Ui::HomeScreen)
{
    ...
    if (isSmallResolution) {
        ...
        ui->atmelPictureLabel->setStyleSheet(QString::fromUtf8("border-image: url(:/resources/smallhome-screen/block-homeautomation.png);"));

        QFont fnt = ui->labelLoading2->font();
        fnt.setPointSize(fnt.pointSize() * wSFactor);

        ui->labelLoading2->setFont(fnt);
    } else {

        mWindow = parent;
        MainWindow* mainWin = (MainWindow*)mWindow;
    ...
    loading = new QMovie(":/resources/other/loader7.gif");
    ui->processLabel->setMovie(loading);
...
/***
 ** Function handler for the Climate Button
 ** Emits a signal to other objects
 ** Reset Controls - shows Main and side control panels
 **/
void HomeScreen::on_pushClimateButton_clicked()
{
    emit(climateRequested());
    emit(resetControls());
}

/***
 ** Function handler for the Lighting Button
 ** Emits a signal to other objects
 ** Reset Controls - shows Main and side control panels
 **/
void HomeScreen::on_pushLightingButton_clicked()
{
    emit(lightingRequested());
    emit(resetControls());
}

/***
 ** Function handler for the Security Button
 ** Emits a signal to other objects
 ** Reset Controls - shows Main and side control panels
 **/
...
/***
 ** Start an hourglass on the home screen
 ** at the application startup
 **/
void HomeScreen::startProgress()
{
    loading->start();
    ui->processLabel->setVisible(true);
    ui->labelLoading2->setVisible(true);
}

/***
 ** Hide a startup hourglass and text
 **/
void HomeScreen::stopProgress()
{
    loading->stop();
    ui->processLabel->setVisible(false);
    ui->labelLoading2->setVisible(false);
//    delete loading;
}
