...
**
** Summary: Qt UI handling class for the MainWindow. All Widgets and Views are nested in this
**          object when showing on LCD display
**
****************************************************************************/
...
/***
 ** Default constructor
 ** - Initialize Primary Screen that will show up at boot time and show it first
 ** with an hourglass and text "Loading..."
 ** - Initialize remaining funtionality in the second step after showing initial screen by
 ** usinge a single shot timer object.
 ** - Connect signals with handlers for initialized objects
 ** - Show the application in the Full Screen Mode
 **/
    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    ui->stackedWidget->setSpeed(transition_speed);
    ...
    if (isSmallResolution) {
        ...
        setMaximumSize(480, 272);
        setGeometry(0, 0, 480, 272);
        ui->centralWidget->setMaximumSize(480, 272);
        ui->centralWidget->setGeometry(0, 0, 480, 272);
        DayWeather::sNormalIconSize = QSize(55,55);
        DayWeather::sLargeIconSize = QSize(130,130);
        qDebug() << "wFactor: " << wSFactor;
        qDebug() << "hFactor: " << hSFactor;
    }
    ...
    //Delay the init of most screens till after I show the initial screen
    QTimer::singleShot(300, this, SLOT(delayedInitProcess()));
}
 ...
 **/
void MainWindow::switchToClimateScreen()
{
...
/***
 ** Handler method for the Lighting button
 ** Switch visible screen to Lighting using custom stackedWidget transition
 ** Set Security and Help screens to a default state (avoid CPU consumption in background)
 ** Update names for Lighting Zones/Rooms
**/
void MainWindow::switchToLightingScreen()
{
	if(lightScreen == NULL)
	{
 ...
 ** Set Security and Help screens to a default state (avoid CPU consumption in background)
 **/
void MainWindow::switchToSecurityScreen()
{
...
/***
 ** Handler method for the Home button
 ** Switch visible screen to Home using custom stackedWidget transition
 ** Set Security and Help screens to a default state (avoid CPU consumption in background)
 **/
...
/***
 ** Delayed Init Process.
 ** Starts when the one time Timer kicked off in Constructor expires
 ** Creates remaining objects for screens and handlers for communication
 **/
...
{
    qDebug("Starting repaint benchmark...");
    MicroCounter counter;
    int i = 100;
    while (i--) {
        update();
        counter.start();
    ...
    qDebug("Average execution of events takes %d [us]", int(counter.averageTime()));
}
