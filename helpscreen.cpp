...
**
** Summary: Qt UI handling class for the Help screen. Please consult helpscreen.ui file for
**          object definitions
**
****************************************************************************/
    ...
    ui(new Ui::HelpScreen)
{
    ...
    if (isSmallResolution) {
        ...
        float wSFactor = 480.0 / 800.;
        float hSFactor = 272.0 / 480.;

        setGeometry(0, 0, this->width() * wSFactor, this->height() * hSFactor);

        ui->label->setPixmap(QPixmap(QString::fromUtf8(":/resources/smalltopnav-all/nav-filler.png")));
        ui->label_2->setPixmap(QPixmap(QString::fromUtf8(":/resources/smalltopnav-all/title-help-demos.png")));

        ui->label->setMinimumSize(ui->label->minimumWidth() * wSFactor, ui->label->minimumHeight() * hSFactor);
        ui->label_2->setMinimumSize(ui->label_2->minimumWidth() * wSFactor, ui->label_2->minimumHeight() * hSFactor);

        ui->buttonHelpClimate->setMinimumSize(ui->buttonHelpClimate->minimumWidth() * wSFactor, ui->buttonHelpClimate->minimumHeight() * hSFactor);
        ui->buttonHelpEntertainment->setMinimumSize(ui->buttonHelpEntertainment->minimumWidth() * wSFactor, ui->buttonHelpEntertainment->minimumHeight() * hSFactor);
        ui->buttonHelpLighting->setMinimumSize(ui->buttonHelpLighting->minimumWidth() * wSFactor, ui->buttonHelpLighting->minimumHeight() * hSFactor);
        ui->buttonHelpSecurity->setMinimumSize(ui->buttonHelpSecurity->minimumWidth() * wSFactor, ui->buttonHelpSecurity->minimumHeight() * hSFactor);
        ui->buttonHelpSettings->setMinimumSize(ui->buttonHelpSettings->minimumWidth() * wSFactor, ui->buttonHelpSettings->minimumHeight() * hSFactor);


        QFont font = ui->buttonHelpClimate->font();
        font.setPointSize(font.pointSize() * wSFactor);
        ui->buttonHelpClimate->setFont(font);
        ui->buttonHelpEntertainment->setFont(font);
        ui->buttonHelpLighting->setFont(font);
        ui->buttonHelpSecurity->setFont(font);
        ui->buttonHelpSettings->setFont(font);

        ui->layoutWidgetBox->setMargin(0);
        ...
        QImage img = QImage(":/resources/smallcamera/up.png");
        QMatrix matrix;
        matrix.rotate(90.);
        img = img.transformed(matrix);
        toggleButton->setIcon(QIcon(QPixmap::fromImage(img)));
//        toggleButton->setIconSize(QSize(24,24));
        toggleButton->setMinimumSize(0,0);
        toggleButton->setMaximumSize(24,24);
        toggleButton->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);

        ui->topMenuLayout->addWidget(toggleButton);
        ui->topMenuLayout->setMargin(0);
        ui->topMenuLayout->setContentsMargins(0, 0, 10, 0);
        ...
        ui->topMenuWidget->resize(450, 40);
        ui->topMenuWidget->move(-ui->topMenuWidget->width() + 40, 0);

        // Change paint order
        ui->topMenuWidget->stackUnder(this);
        ui->widgetGraphicsBox->stackUnder(ui->topMenuWidget);

        QObject::connect(ui->buttonHelpClimate, SIGNAL(clicked()), this, SLOT(runTimer()));
        QObject::connect(ui->buttonHelpEntertainment, SIGNAL(clicked()), this, SLOT(runTimer()));
        QObject::connect(ui->buttonHelpLighting, SIGNAL(clicked()), this, SLOT(runTimer()));
        QObject::connect(ui->buttonHelpSecurity, SIGNAL(clicked()), this, SLOT(runTimer()));
        QObject::connect(ui->buttonHelpSettings, SIGNAL(clicked()), this, SLOT(runTimer()));

        QObject::connect(&timer, SIGNAL(timeout()), this, SLOT(toggleMenus()));

    } else {
        ui->label->setPixmap(QPixmap(QString::fromUtf8(":/resources/topnav-all/nav-filler.png")));
        ui->label_2->setPixmap(QPixmap(QString::fromUtf8(":/resources/topnav-all/title-help-demos.png")));
    }

//    mainControls = new MainControls();
//    ui->help_maincontrols_Layout->addWidget(mainControls);

    mediaplayer = new AtmelMediaPlayer();
    ui->graphicsWidgetLayout->addWidget(mediaplayer);
}
...
/***
 ** Function handler for the Settings help button
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::on_buttonHelpSettings_clicked()
{
    ui->buttonHelpClimate->setChecked(false);
    ui->buttonHelpEntertainment->setChecked(false);
    ui->buttonHelpLighting->setChecked(false);
    ui->buttonHelpSecurity->setChecked(false);
    ui->buttonHelpSettings->setChecked(true);

    mediaplayer->setFileToPlay(QString("./media/file5"));

}

/***
 ** Function handler for the Security help button
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::on_buttonHelpSecurity_clicked()
{
    ui->buttonHelpClimate->setChecked(false);
    ui->buttonHelpEntertainment->setChecked(false);
    ui->buttonHelpLighting->setChecked(false);
    ui->buttonHelpSettings->setChecked(false);
    ui->buttonHelpSecurity->setChecked(true);

    mediaplayer->setFileToPlay(QString("./media/file4"));
}

/***
 ** Function handler for the Entertainment help button
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::on_buttonHelpEntertainment_clicked()
{
    ui->buttonHelpClimate->setChecked(false);
    ui->buttonHelpSettings->setChecked(false);
    ui->buttonHelpLighting->setChecked(false);
    ui->buttonHelpSecurity->setChecked(false);
    ui->buttonHelpEntertainment->setChecked(true);

    mediaplayer->setFileToPlay(QString("./media/file3"));
}

/***
 ** Function handler for the Lighting help button
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::on_buttonHelpLighting_clicked()
{
    ui->buttonHelpClimate->setChecked(false);
    ui->buttonHelpEntertainment->setChecked(false);
    ui->buttonHelpSettings->setChecked(false);
    ui->buttonHelpSecurity->setChecked(false);
    ui->buttonHelpLighting->setChecked(true);

    mediaplayer->setFileToPlay(QString("./media/file2"));
}

/***
 ** Function handler for the Climate help button
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::on_buttonHelpClimate_clicked()
{
    ui->buttonHelpSettings->setChecked(false);
    ui->buttonHelpEntertainment->setChecked(false);
    ui->buttonHelpLighting->setChecked(false);
    ui->buttonHelpSecurity->setChecked(false);
    ui->buttonHelpClimate->setChecked(true);

    mediaplayer->setFileToPlay(QString("./media/file1"));
}

/***
 ** Stops playback of any help media content
 ** Starts media playback of specific help content
 ** Resets button states allowing for a single button in a "pressed" state
 **/
void HelpScreen::stopVideoPlayback()
{
...
void HelpScreen::toggleMenus()
{
    ...
    if (ui->topMenuWidget->pos().x() >= 0) {
        topMenuAnim->setEndValue(QRect(-ui->topMenuWidget->width() + 40, 0, ui->topMenuWidget->width(), ui->topMenuWidget->height()));
...
void HelpScreen::animFinished()
{
    QPushButton *button = ui->topMenuWidget->findChild<QPushButton *>("toggleButton");
