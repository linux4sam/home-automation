...
**
** Summary: This class provides implementation for a custom QTooButton used
**          throughout entire HomeAutomation application
**
****************************************************************************/
...
CustomToolButton::CustomToolButton(QWidget *parent) :
    QToolButton(parent),
    mCheckedMarker(QPixmap(":/resources/topnav-all/icon-active.png"))
{
}

/***
 ** Custom paintEvent handler.
 ** Handler draws a custom rectangle with specific color, sets a picture of a small
 ** orange triangle below text that is set with standard QtooButton methods
 **/
void CustomToolButton::paintEvent(QPaintEvent *event)
{
    if (rect() != event->rect()) //no need to draw partial areas
        return;

    QPainter painter(this);
    QBrush brush(QColor("#151A20"));
    painter.setBrush(brush);
    painter.fillRect(rect(), brush);

   if (isChecked()) {
        painter.setPen(QColor("#f79919"));
        painter.drawText(rect(), Qt::AlignCenter, text());
        painter.drawPixmap( mCheckedMarkerRect, mCheckedMarker);
    } else {
...
void CustomToolButton::resizeEvent( QResizeEvent *event)
{
    ...
    mCheckedMarkerRect.setRect( width() / 2 - mCheckedMarker.width() / 2,
                                 height() - mCheckedMarker.height(),
                                 mCheckedMarker.width(), mCheckedMarker.height());
}
