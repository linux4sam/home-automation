...
**
** Summary: Qt UI handling class for the Media screen. Please consult mediascreen.ui file for
**          object definitions
**
****************************************************************************/

#include "mediascreen.h"
#include "ui_mediascreen.h"
#include "zoneselector.h"
#include <QIcon>

/***
 ** Default constructor
 ** Initializes the custom QuickCast Widget for internet radio playback
 **/
...
{
    ui->setupUi(this);
    ui->toolButtonRadio->setChecked(true);
    mZoneSelector = new ZoneSelector( this);
    mZoneSelector->setBackgroundColor( QColor("#151A20")); //color was hardcoded in customtoolbutton.cpp
    mZoneSelector->setLineColor( QColor("#f79919")); //similar
    QFont font;
    font.setFamily(QString::fromUtf8("Sans Serif"));

    if (isSmallResolution) {
        font.setPointSize(12);
        mZoneSelector->setButtonFont( font);
        mZoneSelector->setTitlePixmap( QPixmap(":/resources/smalltopnav-all/title-media.png"));
        mZoneSelector->setCanHide( true);
        ui->widgetGraphicsBox->stackUnder( mZoneSelector);

        float wSFactor = 480.0 / 800.;
        float hSFactor = 272.0 / 480.;

        QSize iconSize(ui->toolButtonRadio->iconSize().width() * wSFactor, ui->toolButtonRadio->iconSize().height()*hSFactor);
        QIcon iconRadio(QString::fromUtf8(":/resources/smallmediaSelection/button-i-radio-off.png"));
        iconRadio.addFile(QString::fromUtf8(":/resources/smallmediaSelection/button-i-radio-on.png"), iconSize, QIcon::Active, QIcon::On);
        ui->toolButtonRadio->setIcon(iconRadio);
        QIcon iconServer(QString::fromUtf8(":/resources/smallmediaSelection/button-local-server-off.png"));
        iconServer.addFile(QString::fromUtf8(":/resources/smallmediaSelection/button-local-server-on.png"), iconSize, QIcon::Active, QIcon::On);
        ui->toolButtonServer->setIcon(iconServer);
        QIcon iconSirius(QString::fromUtf8(":/resources/smallmediaSelection/button-sirius-xm-off.png"));
        iconSirius.addFile(QString::fromUtf8(":/resources/smallmediaSelection/button-sirius-xm-on.png"), iconSize, QIcon::Active, QIcon::On);
        ui->toolButtonSirius->setIcon(iconSirius);

        ui->toolButtonRadio->setMaximumSize(iconSize.width()+6, iconSize.height()+4);
        ui->toolButtonServer->setMaximumSize(iconSize.width()+6, iconSize.height()+4);
        ui->toolButtonSirius->setMaximumSize(iconSize.width()+6, iconSize.height()+4);
        ui->mediaLayout->setContentsMargins(0,0,0,0);
        ui->verticalLayout_5->setContentsMargins(0, 38*hSFactor,0, 39*hSFactor);

        ui->mediaLayout->setMargin(0);
        ui->verticalLayout_5->setMargin(0);
        ui->horizontalLayout->setMargin(0);
    } else {
        font.setPointSize(16);
        mZoneSelector->setButtonFont( font);
        mZoneSelector->setTitlePixmap( QPixmap(":/resources/topnav-all/title-media.png"));
        ui->verticalLayout->insertWidget( 0, mZoneSelector);
    }

    QStringList zones;
    zones << "Main Floor" << "Second Floor" << "Basement";
    setMediaRoomNames( zones);
    mediaWidget = new QuickCastWidget(isSmallResolution);
    ui->mediaLayout->addWidget(mediaWidget);
}
...
void MediaScreen::resizeEvent(QResizeEvent*)
{
    mZoneSelector->setWidth( width());
}

/***
 ** Handler for the Radio button
 **/
void MediaScreen::on_toolButtonRadio_clicked()
{
    ui->toolButtonRadio->setChecked(true);
    ui->toolButtonServer->setChecked(false);
    ui->toolButtonSirius->setChecked(false);

    //TODO: Add code to handle Internet Radio stream (default funtionality programed)
}

/***
 ** Handler for the SateliteRadio button
 **/
void MediaScreen::on_toolButtonSirius_clicked()
{
    ui->toolButtonRadio->setChecked(false);
    ui->toolButtonServer->setChecked(false);
    ui->toolButtonSirius->setChecked(true);

    //TODO: Add code to handle media playback from a SateliteRadio receiver
}

/***
 ** Handler for the Server button
 **/
void MediaScreen::on_toolButtonServer_clicked()
{
    ui->toolButtonRadio->setChecked(false);
    ui->toolButtonServer->setChecked(true);
    ui->toolButtonSirius->setChecked(false);

    //TODO: Add code to handle media streaming from a local server

}

/***
 ** Set Room Names for Media playback based on Settings panel entries
 **/
void MediaScreen::setMediaRoomNames(QStringList & list)
{
    mZoneSelector->setZoneList( list);
}
